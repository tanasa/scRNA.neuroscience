...
# Sample ID
sample_id <- "jk.18.samples.integrate.SeuratStandard.SCTransform.multiple.cluster.res.normalized_RNA"
...
cat("Loading MTG-SEA reference...\n")
mtg_sea <- readRDS(mtgsea)
gc(verbose = FALSE)
check_ram_and_stop("After loading MTG-SEA reference")
...
if (!exists("seurat_obj")) {
  stop("ERROR: seurat_obj was not created after readRDS()")
}

gc(verbose = FALSE)
check_ram_and_stop("After loading Seurat object")

# Verify object still exists after RAM check
if (!exists("seurat_obj")) {
  stop("ERROR: seurat_obj was lost after RAM check")
}
...
if (!"logcounts" %in% assayNames(sce_obj)) {
  ...
  gc(verbose = FALSE)
  check_ram_and_stop("After normalizing sce_obj")
} else {
...
if (!"logcounts" %in% assayNames(mtg_sea)) {
  ...
  gc(verbose = FALSE)
  check_ram_and_stop("After normalizing MTG-SEA reference")
} else {
...
gc(verbose = FALSE)
check_ram_and_stop("After subsetting to common genes")
...
gc(verbose = FALSE)
check_ram_and_stop("After transferring annotations to full SCE object")
...
if (!"UMAP" %in% reducedDimNames(sce_obj)) {
  ...
  if (!"PCA" %in% reducedDimNames(sce_obj)) {
    ...
    gc(verbose = FALSE)
    check_ram_and_stop("After computing PCA")
  }
  sce_obj <- runUMAP(sce_obj, dimred = "PCA")
  gc(verbose = FALSE)
  check_ram_and_stop("After computing UMAP")
  cat("✓ UMAP computed\n\n")
}
...
dev.off()
gc(verbose = FALSE)
check_ram_and_stop("After generating UMAP plots")
