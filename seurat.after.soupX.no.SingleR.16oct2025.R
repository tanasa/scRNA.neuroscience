...
# sample : 
# sample : soupx_corrected_counts.rds
...
base_dir <- "/mnt/nfs/CX000008_DS1/projects/jaeyeon/fastq_file_Dyslexia_r1/zanalysis_bogdan/fastq_file_Dyslexia_r2_cellRanger"
path <- file.path(base_dir, sample_id)

# Construct path to soupx_corrected folder
soupx_folder <- file.path(path, paste0(sample_id, "_soupx_corrected"))

# Construct the h5 filename from soupx corrected data
h5_file <- file.path(soupx_folder, paste0(sample_id, "_soupx_corrected.h5"))
...
output_dir <- file.path(path, paste0(sample_id, ".soupX.analysis"))
...
# soupX_dir <- file.path(path, paste0(sample_id, ".soupX_results"))
...
# output_dir <- file.path(path, paste0(sample_id, ".analysis"))
...
# rds_file <- "soupx_corrected_counts.rds"
rds_file = h5_file 
...
# Read h5 file
cat("Reading h5 file:", h5_file, "\n")
data_from_h5 <- Read10X_h5(h5_file)

# Check what Read10X_h5 returned
cat("Data structure from h5 file:\n")
str(data_from_h5, max.level = 1)

# Handle different return types from Read10X_h5
if (is.list(data_from_h5)) {
  # If multiple assays (Gene Expression, Antibody Capture, etc.)
  cat("Multiple assays detected. Using 'Gene Expression' if available.\n")
  if ("Gene Expression" %in% names(data_from_h5)) {
    obj <- CreateSeuratObject(counts = data_from_h5[["Gene Expression"]], project = sample_id)
  } else {
    # Use the first assay
    obj <- CreateSeuratObject(counts = data_from_h5[[1]], project = sample_id)
  }
} else if (inherits(data_from_h5, "dgCMatrix") || is.matrix(data_from_h5)) {
  # Single matrix
  message("h5 file contains a matrix; creating a Seurat object...")
  obj <- CreateSeuratObject(counts = data_from_h5, project = sample_id)
} else {
  stop("h5 file returned unexpected format. Class: ",
       paste(class(data_from_h5), collapse = ", "))
}

# Verify Seurat object was created
str(obj)
...
# Calculate percent.mt for human samples (MT- pattern)
obj[["percent.mt"]] <- PercentageFeatureSet(obj, pattern = "^MT-")

# Now check metadata
print(head(obj@meta.data, 3))

# If percent.mt exists, show summary
if ("percent.mt" %in% colnames(obj@meta.data)) {
  cat("\npercent.mt summary:\n")
  print(summary(obj@meta.data$percent.mt))
}

vln_qc <- VlnPlot(obj, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), pt.size = 0.1, ncol = 3)
print(vln_qc)
ggsave(outfn("qc_violin_plot.png"), plot = vln_qc, width = 12, height = 5, dpi = 300)
...
vln_ribo_hb <- VlnPlot(obj, features = c("percent.ribo", "percent.hb", "percent.mt"), pt.size = 0.1, ncol = 3)
print(vln_ribo_hb)
ggsave(outfn("qc_violin_ribo_hb_mt.png"), plot = vln_ribo_hb, width = 12, height = 5, dpi = 300)
...
options(repr.plot.width = 6,   # inches
        repr.plot.height = 6,
        repr.plot.res = 160)   # dpi
elbow_plot <- ElbowPlot(obj, ndims = max(50, npcs))
print(elbow_plot)
ggsave(outfn("pca_elbow_inline.png"), plot = elbow_plot, width = 6, height = 6, dpi = 300)
...
combined_pca <- plot_loadings | plot_dim
print(combined_pca)
ggsave(outfn("PCA_loadings_and_dimplot.png"), plot = combined_pca, width = 14, height = 8, dpi = 300)
...
print(scdblfinder)
ggsave(outfn("scDblFinder_umap.png"), plot = scdblfinder, width = 7, height = 5, dpi = 300)
...
plot_umap_clean = DimPlot(obj_clean, reduction = "umap", label = TRUE)
print(plot_umap_clean)

ggsave(
  filename = outfn("UMAP_post_doublets.png"),
  plot     = plot_umap_clean,
  width    = 7,
  height   = 5,
  dpi      = 300
)
...
# print(cell_cycle)
# ggsave(outfn("cell_cycle_UMAP.png"), cell_cycle, width = 7, height = 5, dpi = 300)

# Cell cycle scoring with error handling
# https://satijalab.org/seurat/articles/cell_cycle_vignette.html

tryCatch({
  cat("\n=== Running Cell Cycle Scoring ===\n")
  
  # Check if cc.genes is available
  if (!exists("cc.genes")) {
    stop("cc.genes not found. This is typically available in Seurat package.")
  }
  ...
  # Create and save cell cycle UMAP plot
  cell_cycle <- DimPlot(obj, reduction = "umap", group.by = "Phase", label = TRUE) + 
    ggtitle("Cell Cycle Phase")
  print(cell_cycle)
  ggsave(outfn("cell_cycle_UMAP.png"), cell_cycle, width = 7, height = 5, dpi = 300)
  cat("Cell cycle UMAP plot saved.\n")
  
  # Show metadata and phase distribution
  cat("\nCell cycle metadata (first few rows):\n")
  print(head(obj@meta.data[, c("S.Score", "G2M.Score", "Phase")]))
  
  cat("\nCell cycle phase distribution:\n")
  print(table(obj@meta.data$Phase))
  
}, error = function(e) {
  cat("\n!!! ERROR in Cell Cycle Scoring !!!\n")
  cat("Error message:", conditionMessage(e), "\n")
  cat("Cell cycle scoring will be skipped.\n")
  cat("Possible reasons:\n")
  cat("  - Some cell cycle genes not found in the dataset\n")
  cat("  - cc.genes object not available\n")
  cat("  - UMAP reduction not present\n\n")
  
  # Continue without cell cycle scoring
  warning("Cell cycle scoring failed. Continuing analysis without it.")
}, warning = function(w) {
  cat("\nWarning in Cell Cycle Scoring:", conditionMessage(w), "\n")
})
...
p_highest <- scater::plotHighestExprs(sce_obj, exprs_values = "counts", n = 50)
print(p_highest) 
ggsave(outfn("highest_exprs_counts.png"), plot = p_highest, width = 8, height = 10, dpi = 300)
...
# Define a helper for safe plotting
safe_plot_gene_umap <- function(obj, gene, outfile) {
  tryCatch({
    cat("Plotting", gene, "...\n")
    p <- plot_gene_umap_seurat(obj, gene)
    print(p)
    ggsave(outfn(outfile), plot = p, width = 12, height = 5, dpi = 300)
    cat("Saved:", outfile, "\n")
  }, error = function(e) {
...
# Run safely for each gene
safe_plot_gene_umap(sce_obj, "APOE",  "gene_APOE_umap_violin.png")
safe_plot_gene_umap(sce_obj, "APP",   "gene_APP_umap_violin.png")
safe_plot_gene_umap(sce_obj, "PSEN1", "gene_PSEN1_umap_violin.png")
safe_plot_gene_umap(sce_obj, "PSEN2", "gene_PSEN2_umap_violin.png")
