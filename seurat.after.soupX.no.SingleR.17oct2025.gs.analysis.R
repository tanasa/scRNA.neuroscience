...
# ============================================================
# ✅ Load the Seurat object from the output directory
# ============================================================

# Define the expected RDS file path
seurat_file <- file.path(output_dir, paste0(sample_id, "_soupX.gs.rds"))

# Check if the file exists
if (!file.exists(seurat_file)) {
  stop("❌ Seurat RDS file not found: ", seurat_file)
}
...
cat("Number of genes (rows):", nrow(obj), "\n")
cat("Number of cells (columns):", ncol(obj), "\n\n")

# ============================================================
# Check for Ensembl-style gene names (ENSG)
# ============================================================

ensembl_genes <- grep("^ENSG", rownames(obj), value = TRUE)
cat("Number of Ensembl-style gene names:", length(ensembl_genes), "\n")

if (length(ensembl_genes) > 0) {
  cat("Examples of Ensembl IDs:\n")
  print(head(ensembl_genes, 10))
} else {
  cat("✅ No Ensembl-style gene names found — all likely converted to gene symbols.\n")
}
...
cat("\nNumber of mitochondrial genes (MT-):", length(mt_genes), "\n")

if (length(mt_genes) > 0) {
  cat("✅ Mitochondrial genes found:\n")
  print(mt_genes)
} else {
  cat("⚠️ No mitochondrial genes found. Check mapping or case sensitivity.\n")
}
...
# Check what assay your object has (usually "RNA")
DefaultAssay(obj)
# [1] "RNA"

# View the metadata for features (genes)
# head(obj[["RNA"]])

# Define analysis subfolder
analysis_dir <- file.path(output_dir, "seurat_analysis")
dir.create(analysis_dir, showWarnings = FALSE, recursive = TRUE)

# Helper function for output filenames
outfn <- function(name) file.path(analysis_dir, paste0(sample_id, "_", name))
...
saveRDS(obj, outfn("seurat_obj.filtered.soupX.gs.rds"))

# SAVE SCE OBJECT

# outfn <- function(name) file.path(output_dir, paste0(sample_id, "_", name))
saveRDS(sce_obj, outfn("sce_obj.filtered.soupX.gs.rds"))
